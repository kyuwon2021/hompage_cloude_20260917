...
function openProduct(key){const p=products.find(p=>p.key===key);if(!p)return;const i=p.detail||info[key]||{en:'BUSINESS SOLUTION',body:p.desc,items:[]};$('#product-detail').innerHTML=`${key==='tv'?`<figure class="product-detail-image"><img src="${esc(safeUrl(p.image))}" alt="Micro RGB TV 대화면 공간 콘셉트"><figcaption>AI 제품 연출 이미지 · 실제 제품 외관과 다를 수 있습니다.</figcaption></figure>`:`<div class="dialog-icon">${icon(key)}</div>`}<p class="eyebrow">${esc(i.en)}</p><h2 id="product-title">${esc(p.title)}</h2><p>${esc(i.body)}</p><ul>${i.items.map(t=>`<li>${esc(t)}</li>`).join('')}</ul>${i.note?`<p class="product-detail-note">${esc(i.note)}</p>`:''}`;$('#product-inquiry').dataset.product=key;$('#product-dialog').setAttribute('aria-labelledby','product-title');$('#product-dialog').showModal();}
